...
public class AdminInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if( session.getAttribute("adminLogin") == null ) { // 1. 비로그인 상태
			logger.info(" >> 접속 불가 : 관리자 비로그인 상태");
			...
			out.println("<script>$(document).ready(function(){swal('관리자 로그인이 필요합니다!', '', 'warning').then(function(){"
