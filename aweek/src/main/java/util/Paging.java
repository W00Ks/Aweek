...
public class Paging {
	...
	private void makePaging() {
		...
		if(totalCount == 0)	return;		// 게시글이 없는 상황에 중단함

		// 기본값 설정
		if(curPage == 0)	setCurPage(1);		// 첫 페이지를 기본 페이지로 설정함
		if(listCount == 0)	setListCount(10);	// 화면에 보여질 게시글 수를 10개로 기본 설정함
		if(pageCount == 0)	setPageCount(10);	// 화면에 보여질 페이지 수를 10개로 기본 설정함
	
		//--------------------
		
		// 총 페이지 수 계산
		totalPage = totalCount / listCount; // 총 게시글 수 / 한 페이지 게시글 수 -> 349 / 10 -> 34
		
		// 총 페이지 수 보정
		// -> 마지막 페이지에 보여질 게시글 수가 listCount보다 작을 때 1페이지 추가하기
		// if(totalPage % listCount > 0) totalPage++; -> 아래것으로 수정
		...
		// 끝 페이지 번호 보정
		// -> 끝 페이지 번호가 마지막 페이지 번호를 넘지 않도록 함
